...
%(5) Define starting paramets for figure
    ...
    height = range(yl)/(sf*2);
    
%(6) Add rectangles and text for each division
    %(Option 1): If plotting by periods
    %(a) Add Era lables
    if strcmpi(divisions,'periods')==1
    for i = 1:length(Name.Eras)
    if strcmpi(Yaxisopt,'normal')
        rectangle(ax,'Position',[Time.Eras(i),ym(1),range(Time.Eras(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Eras(i,:));
        text(ax,Time.Eras(i)+range(Time.Eras(i:i+1))/2,ym(1)+height/2,Name.Eras(i),...
            'HorizontalAlignment','center','FontName','Arial','color','k')
    elseif strcmpi(Yaxisopt,'reverse')
        rectangle(ax,'Position',[Time.Eras(i),yl(2)+range(yl)/(sf*2),range(Time.Eras(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Eras(i,:));
        text(ax,Time.Eras(i)+range(Time.Eras(i:i+1))/2,ym(1)+height/2,Name.Eras(i),...
    ...
    %(b) Add Period labels
    for i = 1:length(Name.Periods)
    if strcmpi(Yaxisopt,'normal')
        rectangle(ax,'Position',[Time.Periods(i),yl(1)-range(yl)/sf+range(yl)/(sf*2),...
            range(Time.Periods(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Periods(i,:));
        if strcmpi(labels,'on')
            text(ax,Time.Periods(i)+range(Time.Periods(i:i+1))/2,yl(1)-range(yl)/sf+range(yl)/(sf*2)+height/2,...
            Name.Periods(i),...
            'HorizontalAlignment','center','VerticalAlignment','middle','FontName','Arial','color','k')
        end
    elseif strcmpi(Yaxisopt,'reverse')
        rectangle(ax,'Position',[Time.Periods(i),yl(2),...
            range(Time.Periods(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Periods(i,:));
        if strcmpi(labels,'on')
            text(ax,Time.Periods(i)+range(Time.Periods(i:i+1))/2,height+height/2,Name.Periods(i),...
            'HorizontalAlignment','center','FontName','Arial','color','k','VerticalAlignment','middle')
        end
    end
    end 
    ...
    for i = 1:length(Name.Periods)
    if strcmpi(Yaxisopt,'normal')
        rectangle(ax,'Position',[Time.Periods(i),ym(1),range(Time.Periods(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Periods(i,:));
        text(ax,Time.Periods(i)+range(Time.Periods(i:i+1))/2,ym(1)+height/2,Abbrev.Periods(i),...
            'HorizontalAlignment','center','FontName','Arial','color','k')
    elseif strcmpi(Yaxisopt,'reverse')
        rectangle(ax,'Position',[Time.Periods(i),yl(2)+range(yl)/(sf*2),range(Time.Periods(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Periods(i,:));
        text(ax,Time.Periods(i)+range(Time.Periods(i:i+1))/2,ym(1)+height/2,Abbrev.Periods(i),...
            'HorizontalAlignment','center','FontName','Arial','color','k','VerticalAlignment','middle')
    end
    ...
    %(b) Add Epoch labels
    for i = 1:length(Name.Epochs)
    if strcmpi(Yaxisopt,'normal')
        rectangle(ax,'Position',[Time.Epochs(i),yl(1)-range(yl)/sf+range(yl)/(sf*2),...
            range(Time.Epochs(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Epochs(i,:));
        if strcmpi(labels,'on')
            text(ax,Time.Epochs(i)+range(Time.Epochs(i:i+1))/2,yl(1)-range(yl)/sf+range(yl)/(sf*2)+height/2,...
            Name.Epochs(i),...
            'HorizontalAlignment','center','VerticalAlignment','middle','FontName','Arial','color','k')
        end
    elseif strcmpi(Yaxisopt,'reverse')
        rectangle(ax,'Position',[Time.Epochs(i),yl(2),...
            range(Time.Epochs(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Epochs(i,:));
        if strcmpi(labels,'on')
            text(ax,Time.Epochs(i)+range(Time.Epochs(i:i+1))/2,height+height/2,Name.Epochs(i),...
            'HorizontalAlignment','center','FontName','Arial','color','k','VerticalAlignment','middle')
        end
    end
    end 
    ...
    %(b) Add Stage labels
    for i = 1:length(Name.Stages)
    if strcmpi(Yaxisopt,'normal')
        rectangle(ax,'Position',[Time.Stages(i),yl(1)-range(yl)/sf+range(yl)/(sf*2),...
            range(Time.Stages(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Stages(i,:));
        if strcmpi(labels,'on')
            lab = char(Name.Stages(i));
            text(ax,Time.Stages(i)+range(Time.Stages(i:i+1))/2,yl(1)-range(yl)/sf+range(yl)/(sf*2)+height/2,...
            lab(1:3),...
            'HorizontalAlignment','center','VerticalAlignment','middle','FontName','Arial','color','k')
        end
    elseif strcmpi(Yaxisopt,'reverse')
        rectangle(ax,'Position',[Time.Stages(i),yl(2),...
            range(Time.Stages(i:i+1)),height],...
            'EdgeColor','k','FaceColor',Color.Stages(i,:));
        if strcmpi(labels,'on')
            text(ax,Time.Stages(i)+range(Time.Stages(i:i+1))/2,height+height/2,Name.Stages(i),...
            'HorizontalAlignment','center','FontName','Arial','color','k','VerticalAlignment','middle')
        end
    end
    end   
